pipeline {
    ...
    stages{
       ...
       stage("Trigger CD Pipeline") {
            steps {
                script {
                    sh "curl -v -k --user Admin:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG=${IMAGE_TAG}' 'ec2-18-234-82-114.compute-1.amazonaws.com:8080/job/register-app-cd/buildWithParameters?token=trigger-token'"
                }
            }
	}
    }
